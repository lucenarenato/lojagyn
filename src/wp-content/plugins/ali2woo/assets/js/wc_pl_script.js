...
(function ($, window, document, undefined) {
    $(function () {
        ...
        $("#doaction, #doaction2").click(function () {
            ...
            if ('a2w_product_update_manual' === check_action) {
                ...
                $('input:checkbox[name="post[]"]:checked').each(function () {
                    ...
                    if (cnt === update_per_request) {
                        ...
                        cnt = 0;
                        data = {'action': 'a2w_sync_products', 'ids': []};
                    }
                });
                ...
                if (total_ids > 0) {
                    a2w_show_block($('.wp-list-table.posts'), sprintf(a2w_wc_pl_script.lang.process_update_d_of_d, 0, total_ids));
                    
                    sprintf(a2w_wc_pl_script.lang.process_update_d_of_d, 0, total_ids)
                    var on_load = function (response_state, state) {
                        if ((state.update_cnt + state.update_error_cnt) === state.num_to_update) {
                            a2w_update_block($(".wp-list-table.posts"), sprintf(a2w_wc_pl_script.lang.complete_result_updated_d_erros_d, state.update_cnt, state.update_error_cnt));
                            //a2w_hide_block($(".wp-list-table.posts"));
                            location.reload();
                        } else {
                            a2w_update_block($(".wp-list-table.posts"), sprintf(a2w_wc_pl_script.lang.process_update_d_of_d_erros_d, state.update_cnt, state.num_to_update, state.update_error_cnt));
                        }
                    };
