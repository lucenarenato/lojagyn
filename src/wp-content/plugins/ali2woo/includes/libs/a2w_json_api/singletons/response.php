...
if (!class_exists('A2W_JSON_API_Response')) {

    class A2W_JSON_API_Response {

        function setup() {
            global $a2w_json_api;
            $this->include_values = array();
            $this->exclude_values = array();
            if ($a2w_json_api->query->include) {
                $this->include_values = explode(',', $a2w_json_api->query->include);
            }
            // Props to ikesyo for submitting a fix!
            if ($a2w_json_api->query->exclude) {
                $this->exclude_values = explode(',', $a2w_json_api->query->exclude);
        ...
        function get_json($data, $status = 'ok') {
            ...
            if (function_exists('json_encode')) {
                ...
                if (version_compare(PHP_VERSION, '5.3') < 0) {
                    ...
                    if ($a2w_json_api->query->json_encode_options) {
                        $json_encode_options = $a2w_json_api->query->json_encode_options;
                    }
                ...
                if (!class_exists('Services_JSON')) {
                    require_once A2W_JSON_API_DIR."/library/JSON.php";
                }
            ...
            if ($a2w_json_api->query->json_unescaped_unicode) {
        ...
        function respond($result, $status = 'ok', $http_status = 200) {
            ...
            if ($a2w_json_api->query->dev || !empty($_REQUEST['dev'])) {
            ...
            } else if ($a2w_json_api->query->redirect) {
                $url = $this->add_status_query_var($a2w_json_api->query->redirect, $status);
                wp_redirect($url);
            } else if ($a2w_json_api->query->callback) {
                // Run a JSONP-style callback with the result
                $this->callback($a2w_json_api->query->callback, $json);
            } else {
                // Output the result
