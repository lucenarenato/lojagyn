<?php
	...
	class DSE_Import {
		...
		/**
		 * Method to fetch a product preview.
		 *
		 */
		public static function Preview_Product() {
			...
			check_admin_referer( 'dse-preview-product-nonce-action', 'dse_preview_product_nonce' );

			$target_product = $_GET[ 'dse_preview_target' ] ?? NULL;
			$product_source = $_GET[ 'dse_preview_source' ] ?? NULL;

			if ( ! $target_product ) {
				wp_die( esc_html__( 'The target product does not seem to be valid. Please refresh the page and try again.', 'dropshipexpress' ) );
			}

			// Set the source
			if ( $product_source && in_array( $product_source, array_keys( self::Get_Sections() ) ) ) {
				$product_source = sanitize_text_field( $product_source );
			} else {
				wp_die( esc_html__( 'Requested API is invalid.', 'dropshipexpress' ) );
			}

			// If the API information are incorrect
			$api = self::Load_API( $product_source );

			if ( FALSE === $api ) {
				wp_die( esc_html__( 'Could not load the API to fetch the preview. Please try again later.', 'dropshipexpress' ) );
			}

			$preview = $api->Fetch_Preview( $target_product );

			// If something went wrong while fetching the preview
			if ( is_wp_error( $preview ) ) {
				wp_die( $preview->last_error() );
			}

			// Load the preview template
			include( DSE_PLUGIN_FOLDER . '/templates/admin/search-import/preview.php' );
		...
		public static function Is_On_Shop( $id, $store ) {
			return self::Is_Imported( $id, $store ) || self::Is_Published( $id, $store );
		}
