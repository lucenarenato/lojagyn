<?php
	...
	class DSE_Api_aliexpress {
		...
		/**
		 * Method to retrieve a product's preview from AlIExpress
		 *
		 * @param $target
		 */
		public function Fetch_Preview( $target ) {

			// Fetch the product
			$product_data = $this->Fetch_Product( $target );
		...
		public function Search( $input, $get = [] ) {
			...
			if ( 3 != $get[ 'dse_product_search_delivery_delay' ] ) {
				$query_args[ 'delivery_delay' ] = (int) $get[ 'dse_product_search_delivery_delay' ];
			}
			...
			// Price values
			if ( 0.0 !== (float) $get[ 'dse_product_search_price_from' ] ) {
				$query_args[ 'price_from' ] = (float) $get[ 'dse_product_search_price_from' ] * 100;
			}

			if ( 0.0 !== (float) $get[ 'dse_product_search_price_to' ] ) {
				$query_args[ 'price_to' ] = (float) $get[ 'dse_product_search_price_to' ] * 100;
			}
			...
			if ( 'yes' === DSE_Settings::Get_Setting( 'aliexpress', 'official_api' ) ) {
				...
				// Query the store
				$response = self::Search_Official( $query_args );
		...
		public static function Search_Official( $query ) {
			...
			// Load the SDK
			if ( defined( 'DSE_PRO_FOLDER' ) && file_exists( DSE_PRO_FOLDER . '/assets/vendors/sdk/aliexpress/TopSdk.php' ) ) {
				require_once( DSE_PRO_FOLDER . '/assets/vendors/sdk/aliexpress/TopSdk.php' );
			} else {
				return new WP_Error( 'dse_missing_file', esc_html__( 'One of the plugin\'s files are missing. Please reinstall the DropShipExpress Pro plugin.', 'dropshipexpress' ) );
			}
			...
			if ( isset( $response->resp_result->result->products->product ) && is_array( $response->resp_result->result->products->product ) ) {
				foreach ( $response->resp_result->result->products->product as $product ) {
					...
					$single_product->discount_perc  = isset( $product->discount ) ? (float) strtok( $product->discount, '%' ) : 0;
					$single_product->thumbnail      = isset( $product->product_main_image_url ) ? $product->product_main_image_url : '';
					$single_product->rating         = isset( $product->evaluate_rate ) ? (float) strtok( $product->evaluate_rate, '%' ) : '50';
