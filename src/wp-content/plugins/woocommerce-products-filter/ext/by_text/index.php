...
final class WOOF_EXT_BY_TEXT extends WOOF_EXT {
    ...
    public function ajax_search() {
        ...
        if (isset($_REQUEST['cur_tax'])) {
            $_GET['really_curr_tax'] = $_REQUEST['cur_tax'];
        }
        $options = [];

        $search_text = sanitize_text_field($_REQUEST['value']);

        $this->options = array_merge($this->options, $this->data_fields($_REQUEST)); //sanitizing is inside
