...
?>
<label class="woof_wcga_label_hide"  for="<?php echo $select_id ?>"><?php echo WOOF_HELPER::wpml_translate($taxonomy_info); ?></label>
<select id="<?php echo $select_id ?>" class="woof_mselect woof_mselect_<?php echo $tax_slug ?>" data-placeholder="<?php echo WOOF_HELPER::wpml_translate($taxonomy_info) ?>" multiple="" size="<?php echo('chosen' == $this->get_select_type() ? 1 : '') ?>" name="<?php echo $this->check_slug($tax_slug) ?>">
