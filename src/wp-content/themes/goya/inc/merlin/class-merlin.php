...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'merlin_admin_footer', array( $this, 'svg_sprite' ) );
	...
	protected function footer() {
		...
		<?php do_action( 'merlin_admin_footer' ); ?>
	...
	?>
		...
		<table class="widefat" cellspacing="0">
			<tbody>
				...
				<?php 
					if ( function_exists( 'phpversion' ) ) {
						...
						$php_check = version_compare($phpversion, '7.4', '>=')  ? 'yes' : 'error'; 
						?>
				<?php if ($php_check == 'error'): ?>
					<tr>
						<td><?php esc_html_e( 'PHP Version', 'goya' ); ?></td>
						<td>
							<span class="dashicons dashicons-warning red"></span> <span><?php echo esc_html($phpversion); ?></span>
							<div><small><?php esc_html_e('Recommended: ', 'goya'); ?> <strong>7.4</strong></small></div>
						</td>
					</tr>
				<?php endif; ?>
				<?php	} ?>
				...
				<?php if ($max_input_vars == 'error'):
				 ...
				 ?>
				<tr>
					<td><?php esc_html_e( 'max_input_vars', 'goya' ); ?></td>
					<td>
						<span class="dashicons dashicons-warning <?php echo esc_attr($color); ?>"></span> <span><?php echo ini_get('max_input_vars'); ?></span>
						<div><small><?php esc_html_e('Recommended: ', 'goya'); ?> <strong>3000</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<td><?php esc_html_e( 'memory_limit', 'goya' ); ?></td>
					<td>
						<span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('memory_limit'); ?></span>
						<div><small><?php esc_html_e('Recommended: ', 'goya'); ?> <strong>256M</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<td><?php esc_html_e( 'max_execution_time', 'goya' ); ?></td>
					<td>
						<span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('max_execution_time'); ?></span>
						<div><small><?php esc_html_e('Recommended: ', 'goya'); ?><strong>300</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'goya' ); ?>:</td>
					<td>
						<span class="dashicons dashicons-warning red"></span> <span><?php echo size_format( wp_max_upload_size() ); ?></span>
						<div><small><?php esc_html_e('Recommended: ' , 'goya'); ?><strong>12M</strong></small></div>
					</td>
				</tr>
				<?php endif; ?>
			...
			<?php endif; ?>
				...
				<?php
					...
					$posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'goya' ) . '">[?]</a>';
		...
		<?php if ( $go == false) { ?>
			<?php if ( $step == 'welcome' ) : ?>
			<p class="server_check_notice step_welcome red"><?php echo sprintf( __( 'Your server does not match the recommended settings. You might not be able to import the full demos.', 'goya' ), '<span class="underline">', '</span>' ); ?></p>
			<p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s, or proceed with caution :(', 'goya' ), '<a href="https://goya.everthemes.com/help-center/server-requirements/" target="_blank">', '</a>' ); ?></p>
			<?php else: ?>
			<p class="server_check_notice step_import red"><?php echo sprintf( __( 'Your server does not match the recommended settings. %sImport the BASIC Demo%s only, or adjust your settings.', 'goya' ), '<span class="underline">', '</span>' ); ?></p>
			<?php endif; ?>
		<?php } ?>
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'merlin-wp' ); ?>">
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<?php foreach ( $this->import_files as $index => $import_file ) : ?>
						<label>
						  <input type="radio" name="demo_import" value="<?php echo esc_attr( $index ); ?>" class="merlin__select-control js-merlin-demo-import-select" <?php echo ($index == 0) ? 'checked = "checked"' : ''; ?>>
						  <img src="<?php echo esc_url( $import_file['import_preview_image_url'] ); ?>">
						  <span><?php echo esc_html( $import_file['import_file_name'] ); ?></span>
						</label>
					<?php endforeach; ?>
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			/**
			* Theme Name: {$parent} Child Theme
			* Description: This is a child theme of {$parent}, automatically generated.
			* Author: {$author}
			* Template: {$slug}
			* Version: {$version}
			*/\n
